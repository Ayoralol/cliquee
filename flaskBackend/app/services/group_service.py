...
def create_group_service(current_user_id, data):
    ...
    if not name or not description:
        return jsonify({'error': 'Missing group name or description'}), 400
...
def search_groups_service(current_user_id, keyword):
    ...
    return jsonify([{'name': group.name, 'description': group.description} for group in groups]), 200
...
def get_group_service(group_id, current_user_id):
    ...
    return jsonify({'name': group.name, 'description': group.description}), 200
...
def get_group_availabilities_service(group_id, current_user_id):
    ...
    return jsonify([{'username': get_username_by_id_service(availability.user_id), 'day': availability.day, 'start_time': availability.start_time, 'end_time': availability.end_time} for availability in all_availabilities]), 200
...
def get_user_availabilities_service(group_id, current_user_id):
    ...
    if not user_group:
        return jsonify({'error': 'Group not found'}), 404
    group = Group.query.filter_by(id=group_id).first()
    return jsonify([{'day': availability.day, 'start_time': availability.start_time, 'end_time': availability.end_time} for availability in group.group_availability]), 200
